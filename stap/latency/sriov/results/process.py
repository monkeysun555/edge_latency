		...
		if "server" in file:
			with open(file_path, 'rb') as f:
				for line in f:
					...
					if not len(parse) == 18:
						continue
					if parse[17] == '102.' and parse[13] == 'first_path1,' and parse[10] == 'netdev_recv,':
					...
					# 	tcp_recv_time = int(parse[1])
						...
						if ingress <= 0 or ingress > MAX_GAP_IN_NS:
							continue
						else:
						...
						if socket_send_time - socket_recv_time > 0 and socket_send_time - socket_recv_time < MAX_GAP_IN_NS/3:
							server_application_time.append(socket_send_time - socket_recv_time)
					elif parse[17] == '116.' and parse[13] == 'first_path1,' and parse[10] == 'net_send,':
						netdev_send_time = int(parse[1])
						egress = netdev_send_time - socket_send_time
						if egress <= 0 or egress > MAX_GAP_IN_NS:
							continue
						else:
